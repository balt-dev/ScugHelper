...
[Tracked]
[CustomEntity("ScugHelper/SandboxedLuaListener")]
public class SandboxedLuaListener(EntityData data, Vector2 _, EntityID id) : Entity(), IAction
{
    ...
    public override void Awake(Scene scene) {
        try
        {
            ...
            if (inlineLua is not null)
                SandboxedLua.LoadInlineString(scene, inlineLua, $"SandboxedLuaListener::{ID}.inline");
            else
                SandboxedLua.LoadFile(scene, filePath, $"SandboxedLuaListener::{ID}");
