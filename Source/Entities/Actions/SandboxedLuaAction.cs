...
[Tracked]
[CustomEntity("ScugHelper/SandboxedLuaAction")]
public class SandboxedLuaAction(EntityData data, Vector2 _, EntityID id) : Entity(), IAction
{
    ...
    public override void Awake(Scene scene) {
        try
        {
            ...
            if (inlineLua is not null)
                SandboxedLua.LoadInlineString(scene, inlineLua, $"SandboxedLuaAction::{ID}.inline");
            else
                SandboxedLua.LoadFile(scene, filePath, $"SandboxedLuaAction::{ID}");
